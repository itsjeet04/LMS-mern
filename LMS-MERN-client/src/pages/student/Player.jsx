...
function Player() {
  const { enrolledCourses, calcChapterTime, backendUrl, getToken, userData, fetchEnrolledCourses } = useContext(AppContext)
  ...
  // Get course data and user's initial rating
  const getCourseData = () => {
    ...
    if (foundCourse) {
      ...
      const userRating = foundCourse.courseRatings.find(r => r.userId === userData._id);
      ...
      <div className="flex flex-col lg:flex-row min-h-screen bg-gray-50">
        <div className="lg:w-1/3 w-full p-4 bg-white border-r border-gray-200 shadow-md">
              ...
              <div key={index} className="bg-white rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                ...
                >
                  <div className="flex-1 text-left">
                    <p className="text-lg font-semibold text-gray-800">{chapter.chapterTitle}</p>
                    <p className="text-sm text-gray-500 mt-1">{chapter.chapterContent.length} lectures • {calcChapterTime(chapter)}</p>
                  </div>
                  <img
                  ...
                  <div className="p-4 border-t border-gray-200 bg-gray-50">
                    <ul className="space-y-3">
                      {chapter.chapterContent.map((lecture, i) => (
                        <li key={i} className="flex items-start gap-3 p-3 rounded-md hover:bg-gray-100 transition">
                          <img
                            src={progressData.includes(lecture.lectureId) ? assets.blue_tick_icon : assets.play_icon}
                            alt="play-icon"
          ...
          <div className="mt-6 mb-4 p-4 bg-white rounded-lg shadow-md border border-gray-200">
            <h1 className="text-xl font-semibold text-gray-800 mb-4">Rate the course:</h1>
          ...
          {playerData ? (
            <div className="space-y-4">
              <div className="aspect-video w-full rounded-lg overflow-hidden shadow-md border border-gray-300">
                <YouTube videoId={playerData.lectureUrl.split('/').pop()} iframeClassName="w-full " />
              </div>
              <div>
                <p className="text-lg font-semibold text-gray-800">{playerData.chapter}.{playerData.lecture} - {playerData.lectureTitle}</p>
              </div>
              <button
              ...
              >
                {progressData.includes(playerData.lectureId) ? 'Completed' : 'Mark as Completed'}
              </button>
            </div>
          ) : (
            <div className="flex justify-center items-center h-full">
              <img src={courseData ? courseData.courseThumbnail : ''} alt="Course Thumbnail" className="rounded-lg shadow-md max-h-[500px] object-contain" />
            </div>
          )}
        </div>
      </div>
